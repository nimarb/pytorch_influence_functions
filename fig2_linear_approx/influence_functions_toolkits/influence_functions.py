...
from fig2_linear_approx.influence_functions_toolkits.hvp_grad import (
    grad_z,
    s_test_sample,
)
...
def calc_influence_single(
    ...
    if s_test_vec is None:
        z_test, t_test = test_loader.dataset[test_id_num] # image, label
        z_test = test_loader.collate_fn([z_test])    # collate_fn is a function that takes a list of samples from dataset and collate them into a batch, return a batched sample
        t_test = test_loader.collate_fn([t_test])
        s_test_vec = s_test_sample(
            model,
            z_test,
